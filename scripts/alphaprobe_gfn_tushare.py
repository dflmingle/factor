...
from alpha_gfn.config import CONSTANTS, DELTA_TIMES, FEATURES, HIDDEN_DIM, OPERATORS  # noqa: E402
...
def build_gfn_components(args: argparse.Namespace, pool: AlphaPoolGFN, device: torch.device) -> tuple[GFNEnvCore, torch.nn.Module, Any, Any, Any, Any, Any]:
    n_tokens = len(FEATURES) + len(OPERATORS) + len(DELTA_TIMES) + len(CONSTANTS)
    backbone = SequenceEncoder(n_tokens, args.encoder_type).to(device)
    env = GFNEnvCore(
        pool=pool,
        encoder=backbone,
        device=device,
